-- test_farm_cactus.lua
-- Runs the real farm_cactus state once, assuming the robot begins at the
-- stasis spot (4,1,3) -- in front of the charging port at (4,1,2).
--
-- Place all split files (common.lua, farm_cactus.lua, etc.) in the same
-- directory as this script and run it from there.
--
-- IMPORTANT: This drives the actual robot. It will go on top of the cactus column and mine down to y=3 as the real
-- state does.
...
local farm_cactus = require("farm_cactus")
...
-- Run the real farm_cactus state exactly once.
local nextState = farm_cactus()

print(string.format("end pos:   x=%d y=%d z=%d facing=%d",
  C.pos.x, C.pos.y, C.pos.z, C.pos.facing))
print("farm_cactus() returned next state: " .. tostring(nextState))
print("=== done ===")
