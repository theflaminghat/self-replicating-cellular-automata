-- test_farm_spruce.lua
-- Runs the real farm_spruce state once, assuming the robot begins at the
-- stasis spot (4,1,3) -- in front of the charging port at (4,1,2).
--
-- Place all split files (common.lua, farm_spruce.lua, etc.) in the same
-- directory as this script and run it from there.
--
-- IMPORTANT: This drives the actual robot. It will take a sapling from the tracked chest, go to the tree base, mine
-- the trunk up and back down, replant, and sweep for items as the real state does.
...
local farm_spruce = require("farm_spruce")
...
-- Run the real farm_spruce state exactly once.
local nextState = farm_spruce()

print(string.format("end pos:   x=%d y=%d z=%d facing=%d",
  C.pos.x, C.pos.y, C.pos.z, C.pos.facing))
print("farm_spruce() returned next state: " .. tostring(nextState))
print("=== done ===")
